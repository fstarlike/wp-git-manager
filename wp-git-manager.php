...
define('GIT_MANAGER_VERSION', '1.4.1');

// Safety: Do NOT perform automatic file-system fixes by default.
// To enable auto-fix behavior (not recommended on production servers),
...
function git_manager_enqueue_global_checker()
{
    ...
    if ($has_access) {
        wp_enqueue_script('git-manager-global', plugin_dir_url(__FILE__).'admin/git-manager-global.js', ['wp-i18n'], GIT_MANAGER_VERSION, true);
...
function git_manager_enqueue_admin_assets($hook)
{
    ...
    if (strpos($hook, 'git-manager') === false) {
        return;
    }
